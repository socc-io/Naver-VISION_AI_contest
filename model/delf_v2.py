...
_SUPPORTED_TARGET_LAYER = ['resnet_v2_50/block3', 'resnet_v2_50/block4']
...
class DelfV2(object):
  ...
  def __init__(self, target_layer_type=_SUPPORTED_TARGET_LAYER[0], skipcon_attn=False):
  ...
  def _PerformAttention(self,
    ...
    with tf.variable_scope(
      ...
      with tf.variable_scope(
        ...
        if self.skipcon_attn:
            feature_map_sum = tf.expand_dims(tf.expand_dims(
                tf.reduce_mean(feature_map, [1,2]), 1), 2)
            attention_feat += feature_map_sum
  ...
  def GetResnet50Subnetwork(self,
                            ...
                            global_pool=False,
                            reuse=None):
    """Constructs resnet_v2_50 part of the DELF model.
  ...
  def AttentionModel(self,
    ...
    """
    if 'resnet_v2_50' in self._target_layer_type:
